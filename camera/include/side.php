<div class="sidebar">
            <nav class="sidebar-nav">
                <ul class="nav">
                    <li class="nav-item">
                        <a class="nav-link" href="index.php"><i class="icon-speedometer"></i><?=$active?></a>
                    </li>

                    <li class="nav-title">
                        İşlemler
                    </li>
                    <li class="nav-item nav-dropdown">
                        <a class="nav-link nav-dropdown-toggle" href="#"><i class="icon-puzzle"></i> Araçlar</a>
                        <ul class="nav-dropdown-items">
                          
                        <li class="nav-item">
                        <a class="nav-link" href="index.php?islem=araclar"><i class="icon-puzzle"></i>Araçlar</a>
                    </li>              
                        </ul>
                    </li>
                    <li class="nav-item nav-dropdown">
                        <a class="nav-link nav-dropdown-toggle" href="#"><i class="icon-star"></i> Talepler</a>
                        <ul class="nav-dropdown-items">
                            <li class="nav-item">
                                <a class="nav-link" href="index.php?islem=talep-ekle"><i class="icon-star"></i>Talep Ekle</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="index.php?islem=talepler"><i class="icon-star"></i>Talepler</a>
                            </li>
                        </ul>
                    </li>
                    <li class="nav-item nav-dropdown">
                    <a class="nav-link nav-dropdown-toggle" href="#"><i class="icon-wrench"></i><strong> Arızalar </strong></a>
                        <ul class="nav-dropdown-items">
                    

                            <li class="nav-item">
                                <a class="nav-link" href="index.php?islem=ariza-ekle"><i class="icon-wrench"></i>Arıza Ekle</a>
                            </li>

                            <li class="nav-item">
                            <a class="nav-link" href="index.php?islem=arizalar"><i class="icon-wrench"></i>Arızalar</a>
                        </li>
                        </ul>
                    </li>
                   
                    <li class="nav-item nav-dropdown">
                    <a class="nav-link nav-dropdown-toggle" href="#"><i class="icon-star"></i>Evrak Kayıt</a>
                        <ul class="nav-dropdown-items">
                            <li class="nav-item">
                                <a class="nav-link" href="icons-font-awesome.html"><i class="icon-star"></i>Evrak Ekle</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="icons-simple-line-icons.html"><i class="icon-star"></i>Evraklar</a>
                            </li>
                        </ul>
                    </li>
                  
                  

                </ul>
            </nav>
        </div>